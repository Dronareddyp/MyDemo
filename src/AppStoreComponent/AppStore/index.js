...
class AppStore extends Component {
  ...
  onIdValue = appId => {
    this.setState({appListId: appId})
  }

  onFiltering = filterCharecter => {
    const {appListId} = this.state
    const initiateFilter = filterCharecter.filter(
      items => items.category === appListId,
    )
    return initiateFilter
  }
  onFindCahr = () => {
    const {searchInput} = this.state
    const filterCharecter = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return filterCharecter
  }

  render() {
    const {appListId, searchInput} = this.state
    const filterCharecter = this.onFindCahr()
    const initiateFilter = this.onFiltering(filterCharecter)
    return (
      <div>
        <div>
          ...
          <ul>
            {tabsList.map(demo => (
              <TabItem
                key={demo.tabId}
                listItem={demo}
                clickItems={this.onIdValue}
                isActive={appListId === demo.tabId}
              />
            ))}
          </ul>
          <ul className="db-container">
            {initiateFilter.map(eachapp => (
              <AppItem key={eachapp.appId} listItems={eachapp} />
            ))}
          </ul>
        </div>
      </div>
