...
class MatchGame extends Component{
    state={
        isTrue:false,
        imageTumbnail:imagesList[0].imageUrl,
        score:0,
        timer:60,
        category:"FRUIT",
    }
    ...
    imageClick=(imgValue)=>{
      const {imageTumbnail} = this.state
      const filterImage = imagesList.filter((each)=>each.thumbnailUrl===imgValue)
      console.log("filterImage",filterImage)
      const {imageUrl} = filterImage[0]
      console.log("imageUrl", imageUrl)
      if(imageUrl===imageTumbnail){
        const newImageUrl=
        imagesList[Math.floor(Math.random()* imagesList.length)].imageUrl
        this.setState((prev)=>({
          score:prev.score + 1,
          imageTumbnail:newImageUrl
        }))
      }else{
        clearInterval(this.timerId)
        this.setState({isTrue:true})
    ...
    render(){
        const {timer, category, isTrue, score, imageTumbnail} =this.state
        const imageEmaId = imagesList.filter((each)=>each.category===category)
        // const imageEmaId = this.onclickTablist()
        return(
            <div className="db_container">
                <nav className="navBarheder">                    
                    <img className="imgesMatchGame" 
                    src="https://assets.ccbp.in/frontend/react-js/match-game-website-logo.png" 
                    alt="website logo"/>
                    <ul className="time_container">
                        <li><p>Score: {score}</p></li>
                        <li>
                        <img className="imagesS"
                        src="https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png" 
                        alt="timer"/>  <span>{timer} sec</span>  
                        </li>                    
                    </ul>
                    </nav>
                    {!isTrue&&(
                      <div>
                        <img className="imageTumbnail" src={imageTumbnail} alt="match"/>
                    <div>
                        <ul className="tabList">
                            {tabsList.map((demo)=>(
                                <li key={demo.tabId}>
                                    <button type="button" className={`tab-button 
                                    ${category===demo.tabId?"highlight-text":""}`}
                                     onClick={()=>this.onclicUpdate(demo.tabId)}>
                                    {demo.displayText}
                                    </button>
                                </li>
                            ))}
                        ...
                        {imageEmaId.map((demo)=>(
                            <li key={demo.id} >
                              <button type="button" onClick={()=>this.imageClick(demo.thumbnailUrl)}>
                                <img className="tumbnailUrl" src={demo.thumbnailUrl} alt="thumbnailUrl"/>
                                </button>
                            </li>                            
                        ))}
                        </ul>
                    ...
                    {isTrue&&(
                      <div className="imageGameOver">
                        <img className="tropyImage"
                        src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png" 
                        alt="trophy"/>
                        <p>YOUR SCORE</p>
                        <p>{score}</p>
                        <button type="button" className="button" onClick={this.resetValue}>
                          <img className=""
